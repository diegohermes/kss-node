...
KssSection.prototype.JSON = function () {
	var modifiers, parameters;

	modifiers = this.data.modifiers.map(function(modifier) {
		return {
			name: modifier.name(),
	...
	parameters = this.data.parameters.map(function(modifier) {
		return {
			name: modifier.name(),
			description: modifier.description()
		};
